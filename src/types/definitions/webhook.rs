...
use crate::{
    ...
    types::{AllowedUpdate, InputFileReader, Integer},
};
...
pub struct SetWebhook {
    certificate: Option<InputFileReader>,
...
impl SetWebhook {
    ...
    {
        self.parameters.allowed_updates = Some(value.into_iter().collect());
        self
    }
    ...
    /// * `value` - Indicates whether to drop all pending updates.
    pub fn with_drop_pending_updates(mut self, value: bool) -> Self {
        self.parameters.drop_pending_updates = Some(value);
        self
    }
    ...
    where
        T: Into<String>,
    {
        self.parameters.ip_address = Some(value.into());
        self
    }
    ...
    pub fn with_max_connections(mut self, value: Integer) -> Self {
        self.parameters.max_connections = Some(value);
        self
    }
    ...
    where
        T: Into<String>,
    {
        self.parameters.secret_token = Some(value.into());
        self
    }
}
...
impl Method for SetWebhook {
    ...
    fn into_payload(self) -> Result<Payload, PayloadError> {
        ...
        parameters.certificate = certificate.map(|x| x.write(&mut form));
        parameters.serialize(&mut form)?;
        Payload::form("setWebhook", form)
    }
}
